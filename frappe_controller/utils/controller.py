...
from frappe.utils import cint, get_bench_path, get_sites, now_datetime
...
def wait_for_event(event_key: str, condition: str | None = None, consider_events_since: str | None = None) -> dict:
	"""
	...
	events = frappe.get_all(
		"FS Event", filters=filters, fields=["argument", "creation"], order_by="creation asc"
	)
...
def emit_event(key: str, argument: dict | None = None):
	"""
		...
		for msg_id, payload in stream_messages:
			...
			if stream_name == "fs:events":
				...
				for cond in conditions:
					...
					# Evaluate condition
					if not cond.condition or frappe.safe_eval(
						cond.condition, None, {"argument": event_argument}
					):
						# Satisfy condition
